# -*- coding: utf-8 -*-
"""
Created on Wed Sep 14 18:52:30 2022
...
# for example, seed(1) - the number inside this bracket is to remain same


seed(33)

# objective function of two variables, considered here for maximization
def objective(x):
	return x[0]**2.0 + x[1]**3.0



# define range for the inputs, i.e., x[0] and x[1]
...
# Tournament selection [there are other selection strategies too]
...
# the operation of crossover and mutation are done in bit string; but, the objective function is in real-number space; so, we need to decode the binary encoding to real number            
...
def ga(objective, x_bounds, n_bits, n_iteration, n_agents, rate_crossover, rate_mutation):
	...
	best, best_obj = 0, objective(convert(x_bounds, n_bits, pop_agents[0]))  # initializing the best evaluated objective function (thus far) with the first agent [following the conversion to decimal number]
	...
	for k in range(n_iteration):     # for the specified number of iterations, ideally, it should be made to stop when convergence has been achieved
		...
		decoded = [convert(x_bounds, n_bits, p) for p in pop_agents] # all the agents' bit strings converted to real values
...
print('f(%s) = %f' % (best_realvalue, best_obj)) # the answer (optima) to the objective function